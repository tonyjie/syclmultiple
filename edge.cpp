...
// ========== Q1 setting prepare ==========
  ...
  auto inImgHeight_a = inImgHeight_tot / 2; // split the image in half
  inImgHeight_a = inImgHeight_a / 32 * 32;  // make it a multiple of 32
  auto inImgHeight_b = inImgHeight_tot - inImgHeight_a;
...
// ========= Q2 measurement end ==========
...

  {
    ...
    sycl::event e3 = myQueue2.submit([&](sycl::handler& cgh1) {
...
// ======= Q1 measurement ==========
    ...
    std::cout << "====== Picture Blurring (Queue 1 & 2) =======\n";
    std::cout << "profiling: Operation completed on device1 in " << time1A
              << " nanoseconds (" << time1A / 1.0e9 << " seconds)\n";
    std::cout << "profiling: Operation completed on device2 in " << time1B
              << " nanoseconds (" << time1B / 1.0e9 << " seconds)\n";
    std::cout << "chrono: Operation completed on device1 and device 2 in " << time1C * 1000
              << " nanoseconds (" << time1C * 1000 / 1.0e9 << " seconds)\n";
